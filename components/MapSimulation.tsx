  ...
  return (
    <div className="fixed inset-0 bg-gray-100 z-50 flex flex-col">
      ...
      <div ref={mapContainerRef} className="flex-1 relative overflow-auto bg-[#eef2f6] touch-pan-x touch-pan-y pb-48">
        ...
        {/* LARGE MAP CANVAS */}
        <div className="relative w-[1200px] h-[900px] bg-white shadow-2xl m-4 border-8 border-gray-300 rounded-xl overflow-hidden">
            ...
            {/* Cafeteria (Comedor) */}
            <div className="absolute top-[20%] left-[15%] w-[20%] h-[15%] bg-blue-600 rounded-lg shadow-lg flex flex-col items-center justify-center border-b-4 border-blue-800">
                <Warehouse className="text-white/80 mb-1" size={32} />
                <span className="text-sm text-white font-bold">COMEDOR</span>
            </div>

            {/* Football Field */}
            <div className="absolute top-[40%] left-[10%] w-[25%] h-[20%] bg-green-600 rounded-lg border-4 border-white/80 shadow-inner flex items-center justify-center overflow-hidden">
                {/* Field markings */}
            ...
            {/* ETI Prefab Classrooms (Moved Up) */}
            <div className="absolute top-[63%] left-[10%] w-[30%] h-[10%] bg-yellow-400 rounded-md shadow-md flex items-center justify-center border border-yellow-500">
            ...
            {/* Parking */}
            <div className="absolute top-[10%] left-[60%] w-[30%] h-[12%] bg-gray-300 rounded-lg border-2 border-gray-400 flex items-center justify-center">
                 <div className="grid grid-cols-6 gap-2 opacity-30">
                    {[...Array(12)].map((_, i) => <div key={i} className="w-6 h-8 border border-gray-500"></div>)}
                 </div>
                 <span className="absolute text-xs text-gray-600 font-bold bg-gray-200 px-1">ESTACIONAMIENTO</span>
            </div>

            {/* Auditorio */}
            <div className="absolute top-[25%] left-[60%] w-[25%] h-[12%] bg-blue-600 rounded-lg shadow-lg flex flex-col items-center justify-center border-b-4 border-blue-800">
                <Building className="text-white/80 mb-1" size={24} />
                <span className="text-sm text-white font-bold">AUDITORIO</span>
            </div>

            {/* Central Park */}
            <div className="absolute top-[40%] left-[45%] right-[35%] h-[10%] z-10 flex justify-center gap-4 pt-2">
                 <div className="bg-green-100 p-2 rounded-full border border-green-300 shadow-sm"><Trees className="text-green-600" size={24} /></div>
                 <div className="bg-green-100 p-2 rounded-full border border-green-300 shadow-sm"><Trees className="text-green-600" size={24} /></div>
            </div>

            {/* Sports Court (Loza) */}
            <div className="absolute top-[40%] left-[60%] w-[15%] h-[20%] bg-orange-100 rounded border-2 border-orange-300 flex items-center justify-center">
                 <div className="w-[80%] h-[90%] border-2 border-orange-400"></div>
                 <span className="absolute text-xs text-orange-800 font-bold rotate-90">LOZA DEPORTIVA</span>
            </div>

            {/* --- TORRES TECNOLÓGICAS (Bottom Right - Moved Up for Visibility) --- */}
            
            {/* Zone Label */}
            <div className="absolute top-[70%] left-[65%] text-xs font-bold text-gray-400 tracking-widest uppercase">
                Complejo Tecnológico
            </div>

            {/* TORRE A */}
            <div className={`absolute bottom-[10%] left-[66%] w-[10%] h-[12%] flex flex-col items-center justify-center rounded-lg shadow-lg border-2 transition-all ${isTowerA ? 'bg-yellow-300 ring-4 ring-senati-accent scale-105 z-20' : 'bg-yellow-400 border-yellow-600'}`}>
            ...
            {/* TORRE B */}
            <div className={`absolute bottom-[10%] left-[78%] w-[10%] h-[12%] flex flex-col items-center justify-center rounded-lg shadow-lg border-2 transition-all ${isTowerB ? 'bg-yellow-300 ring-4 ring-senati-accent scale-105 z-20' : 'bg-yellow-400 border-yellow-600'}`}>
            ...
            {/* TORRE C */}
            <div className={`absolute bottom-[10%] left-[90%] w-[10%] h-[12%] flex flex-col items-center justify-center rounded-lg shadow-lg border-2 transition-all ${isTowerC ? 'bg-yellow-300 ring-4 ring-senati-accent scale-105 z-20' : 'bg-yellow-400 border-yellow-600'}`}>
